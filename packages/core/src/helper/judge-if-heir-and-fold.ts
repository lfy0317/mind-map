...
export const judgeIfHeirAndFold = (
  ...
  if (isRootChild && root.fold) {
    // negative 折叠，并且节点为root直系negative
    if (root.fold[0] && getRootHeirOrientation(node.id) === Mind.Orientation.negative) {
      return true
    }

    // positive 折叠，并且节点为root直系positive
    if (root.fold[1] && getRootHeirOrientation(node.id) === Mind.Orientation.positive) {
      return true
    }
  }
