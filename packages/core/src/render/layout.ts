...
export const layout = (context: {
  ...
  const { cacheMap, options, root, container, viewport, transform, anchor, zoom, center } = context
  ...
  Helper.depthFirstWalkTree(shadowNode, {
    ...
    before: (node, parent) => {
      ...
      return !Helper.judgeIfAllChildFold(node, root.id)
    }
  })
