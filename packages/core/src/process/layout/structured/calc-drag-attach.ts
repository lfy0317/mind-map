...
export const calcDragAttach: typeof DraggableLayout.calcDragAttach = (context) => {
  ...
  let orientation: Mind.Orientation = Mind.Orientation.root
  const rootRect = cacheMap.get(root.id)!.rect
  const targetLinkPoint: Mind.Coordinate = {
    x: 0,
    y: 0
  }
  ...
  let linkNode: Mind.Node | undefined = undefined
  let linkPoint: Mind.Coordinate | undefined = undefined
  ...
  for (const node of canBeAttachedNodes) {
    ...
    const canBeLinkPoint: Mind.Coordinate = {
      x: 0,
      y: 0
    }
    ...
    // if (
    //   targetLinkPoint.y > rect.y + rect.height
    //   && targetLinkPoint.y < rect.y - rect.height
    //   && targetLinkPoint.x > rect.x + rect.width
    //   && targetLinkPoint.x < rect.x - rect.width
    // ) {
    //   minMainDistance = 0
    //   linkNode = node
    ...
    if (root.id === node.id) {
      if (
        // 调整拖拽时的间距灵敏度
        targetCrossValue < crossBoundary.min - options.nodeSeparate! * 4 ||
        targetCrossValue > crossBoundary.max + options.nodeSeparate! * 4
      ) {
        continue
      }
    } else {
  ...
  if (linkPoint && linkNode) {
    let children: Mind.Node[] = linkNode.children || []

    // 根节点折叠特殊处理
    if (linkNode.id === root.id) {
      const fold = root.fold || []
