...
export class Visible implements Process.Lifecycle<void, void, void, Mind.Visible> {
   ...
   */
    ...
    const viewportSize: Mind.Size = {
      height: this.viewport.clientHeight * 2,
      width: this.viewport.clientWidth * 2
    }
    // 屏幕坐标转换到绘制坐标系
    ...
    // 广度优先搜索
    Helper.breadthFirstWalkTree(this.cacheMap.get(this.root.id)!.node, {
      after: () => {},
      before: (node) => {
        ...
        // 折叠则不处理
        return !Helper.judgeIfAllChildFold(node, this.root.id)
      }
    })
